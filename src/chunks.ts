...
export const getCopyMask = (start: number, end: number, bitLength: number): string => {
...
export const combinePartials = (left: PartialChunk, right: PartialChunk): number => {
  if (left.length + right.length != 5) {
    throw new Error(`Combined length of partial chunks must be 5 (${left.length} + ${right.length} != 5)`);
  }
  const lVal = sigBits(left.bits) === left.length ? left.bits << 5 - left.length : left.bits << sigBits(left.bits);
  const rVal = sigBits(right.bits) === right.length ? right.bits >>> 5 - right.length : right.bits >>> sigBits(right.bits);
...
export const byteToChunk = (val: number, mask: string): number => {
  ...
  const trailingZeroIndex = mask.replace(/^0+/, '-').indexOf('0'); // we will get -1 if no trailing zeros
...
export const toChunks = (uint8: Uint8Array): number[] => {
  ...
  uint8.forEach((byte, idx) => {
    ...
    if (startChunk + 5 <= 8) {
      let mask = getCopyMask(startChunk, endChunk, 8);
    ...
    if (startChunk < 8) { // we have a 'left' partial
      let mask = getCopyMask(startChunk, endChunk, 8);
      if (idx == uint8.length - 1) { // we are out of bytes so pad this chunk out
